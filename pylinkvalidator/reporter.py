...
def _print_csv_details(page_iterator, output_files, config):
    for page in page_iterator:
        content_messages = ""
        for content_message in page.get_content_messages():
            content_messages += "{0},".format(content_message.replace(",", ""))
        source_details = ""
        for source in page.sources:
            source_details = "{0},".format(source.origin.geturl())
            if config.options.show_source:
                source_details += "{0},".format(truncate(source.origin_str).replace(",", ""))
        oprint("{0},{1},{2}{3}".format(page.get_status_message(), page.url_split.geturl(), content_messages, source_details), files=output_files)
